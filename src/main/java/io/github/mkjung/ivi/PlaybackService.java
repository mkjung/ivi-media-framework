        ...
        public void onEvent(MediaPlayer.Event event) {
            switch (event.type) {
                case MediaPlayer.Event.Playing:
                    ...
                    if (!mWakeLock.isHeld())
                        mWakeLock.acquire();
                    if (!keyguardManager.inKeyguardRestrictedInputMode() && !mVideoBackground
//                            && switchToVideo()
                            )
                        hideNotification(false);
                    else
...
//                    updateWidgetPosition(event.getPositionChanged());
                ...
                case MediaPlayer.Event.ESAdded:
                    if (event.getEsChangedType() == Media.Track.Type.Video && (mVideoBackground
//                            || !switchToVideo()
                    )) {
